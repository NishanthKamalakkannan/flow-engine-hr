  ...
  return (
    <div className="fixed top-14 bottom-0 right-0 w-[440px] bg-[#0D1420] border-l border-[#1F2937] z-50 flex flex-col shadow-[-20px_0_60px_rgba(0,0,0,0.9)]">
      ...
      <div className="flex items-center justify-between px-6 py-6 border-b border-[#1F2937] bg-[#080C14]/50">
        ...
        <button onClick={handleClose} className="text-[#6B7280] hover:text-[#F9FAFB] transition-colors p-2 hover:bg-[#1F2937] rounded-xl">
          <X size={24} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-8 flex flex-col gap-6 bg-[#0D1420]">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-16 gap-5">
            <div className="relative">
              ...
              <div className="w-14 h-14 border-t-4 border-[#A855F7] rounded-full animate-spin absolute top-0 left-0" />
            </div>
        ...
        {isFinished && result?.success && (
          <div className="bg-[#10B981]/10 border border-[#10B981]/30 rounded-2xl p-6 flex flex-col items-center text-center gap-4">
            <div className="w-14 h-14 rounded-2xl bg-[#0D1420] flex items-center justify-center text-[#10B981] border border-[#10B981]/30">
              <Trophy size={28} />
            </div>
            <div>
            ...
            <div className="flex flex-col gap-4">
              {result.steps.map((step: any, i: number) => (
                <div
                  key={i}
                  className={clsx(
                    "flex items-start gap-5 p-5 rounded-2xl border transition-all duration-700",
                    i <= activeStep ? "bg-[#111827] border-[#1F2937] opacity-100" : "opacity-10 border-transparent scale-95",
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2">
                      <p className="text-[#F9FAFB] text-[13px] font-black truncate tracking-wide uppercase leading-none">{step.label}</p>
                      <span className={clsx("text-[8px] px-2.5 py-1 rounded-lg border font-black tracking-widest uppercase", nodeTypeColor[step.nodeType] || "")}>
                        {step.nodeType}
                      </span>
                    </div>
                    ...
                    {step.details && i <= activeStep && (
                      <button
                        ...
                        className="mt-5 flex items-center gap-2.5 text-[#A855F7] text-[10px] font-black hover:text-[#7C3AED] transition-colors uppercase tracking-widest bg-[#080C14] px-3 py-1.5 rounded-xl border border-[#1F2937]"
                      >
                    ...
                    {expandedDetails[i] && step.details && (
                      <div className="mt-5 p-5 bg-black/40 rounded-2xl border border-[#1F2937] font-mono text-[10px] overflow-x-auto">
        ...
        {!result && !loading && validationMessages.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 gap-6 text-center">
            <div className="w-20 h-20 rounded-3xl bg-[#080C14] flex items-center justify-center border border-[#1F2937]">
              <Play size={32} className="text-[#7C3AED] ml-1 fill-current" />
            </div>
            <div>
      ...
      <div className="p-4 border-t border-[#1F2937] bg-[#080C14]/80 flex-shrink-0">
        <button
          onClick={handleSimulate}
          disabled={loading}
          className="w-full flex items-center justify-center gap-4 py-5 rounded-2xl bg-[#7C3AED] hover:bg-[#A855F7] disabled:opacity-20 text-white font-black text-[12px] uppercase tracking-[0.2em] transition-all shadow-xl shadow-purple-900/40 active:scale-[0.98] border border-purple-400/30"
        >
